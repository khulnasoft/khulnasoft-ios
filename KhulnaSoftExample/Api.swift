...
struct KhulnaSoftBeerInfo: Codable, Identifiable {
    let id: Int
    var name: String
...
class Api: ObservableObject {
    @Published var beers = [KhulnaSoftBeerInfo]()

    var logger = Logger(subsystem: Bundle.main.bundleIdentifier!, category: "main")

    func listBeers(completion _: @escaping ([KhulnaSoftBeerInfo]) -> Void) {
...
//            let beers = try! JSONDecoder().decode([KhulnaSoftBeerInfo].self, from: data!)
//
