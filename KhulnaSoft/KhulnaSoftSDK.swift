...
// renamed to KhulnaSoftSDK due to https://github.com/apple/swift/issues/56573
@objc public class KhulnaSoftSDK: NSObject {
    private var config: KhulnaSoftConfig

    private init(_ config: KhulnaSoftConfig) {
        self.config = config
    }
    ...
    private var queue: KhulnaSoftQueue?
    private var replayQueue: KhulnaSoftQueue?
    private var storage: KhulnaSoftStorage?
    #if !os(watchOS)
        private var reachability: Reachability?
    #endif
    private var flagCallReported = Set<String>()
    private var featureFlags: KhulnaSoftFeatureFlags?
    private var context: KhulnaSoftContext?
    private static var apiKeys = Set<String>()
    private var capturedAppInstalled = false
    private var appFromBackground = false
    private var isInBackground = false
    #if os(iOS)
        private var replayIntegration: KhulnaSoftReplayIntegration?
    #endif
    /// Internal, only used for testing
    var shouldReloadFlagsForTesting = true

    #if os(iOS) || targetEnvironment(macCatalyst)
        private var autocaptureIntegration: KhulnaSoftAutocaptureIntegration?
    #endif
    // nonisolated(unsafe) is introduced in Swift 5.10
    #if swift(>=5.10)
        @objc public nonisolated(unsafe) static let shared: KhulnaSoftSDK = {
            let instance = KhulnaSoftSDK(KhulnaSoftConfig(apiKey: ""))
            return instance
        }()
    #else
        @objc public static let shared: KhulnaSoftSDK = {
            let instance = KhulnaSoftSDK(KhulnaSoftConfig(apiKey: ""))
            return instance
        }()
    #endif
    ...
    @objc public func setup(_ config: KhulnaSoftConfig) {
        setupLock.withLock {
            ...
            if KhulnaSoftSDK.apiKeys.contains(config.apiKey) {
                hedgeLog("API Key: \(config.apiKey) already has a KhulnaSoft instance.")
            } else {
                KhulnaSoftSDK.apiKeys.insert(config.apiKey)
            }

            enabled = true
            self.config = config
            let theStorage = KhulnaSoftStorage(config)
            storage = theStorage
            let api = KhulnaSoftApi(config)
            featureFlags = KhulnaSoftFeatureFlags(config, theStorage, api)
            config.storageManager = config.storageManager ?? KhulnaSoftStorageManager(config)
            #if os(iOS)
                replayIntegration = KhulnaSoftReplayIntegration(config)
            #endif

            #if os(iOS) || targetEnvironment(macCatalyst)
                autocaptureIntegration = KhulnaSoftAutocaptureIntegration(config)
            #endif
                ...
                context = KhulnaSoftContext(reachability)
            #else
            ...
            #if !os(watchOS)
                queue = KhulnaSoftQueue(config, theStorage, api, .batch, reachability)
                replayQueue = KhulnaSoftQueue(config, theStorage, api, .snapshot, reachability)
            #else
                queue = KhulnaSoftQueue(config, theStorage, api, .batch)
                replayQueue = KhulnaSoftQueue(config, theStorage, api, .snapshot)
            #endif
            ...
            KhulnaSoftSessionManager.shared.startSession()
            ...
            DispatchQueue.main.async {
                NotificationCenter.default.post(name: KhulnaSoftSDK.didStartNotification, object: nil)
            }
    ...
    @objc public func getSessionId() -> String? {
        ...
        return KhulnaSoftSessionManager.shared.getSessionId()
    }
    ...
    @objc public func endSession() {
        ...
        KhulnaSoftSessionManager.shared.endSession {
            self.resetViews()
        }
    }
                                 ...
                                 appendSharedProps: Bool = true) -> [String: Any]
    {
        ...
        if let sessionId = KhulnaSoftSessionManager.shared.getSessionId() {
        ...
        // remove after https://github.com/KhulnaSoft/khulnasoft/pull/18954 gets merged
    ...
    private func isOptOutState() -> Bool {
        if config.optOut {
            hedgeLog("KhulnaSoft is in OptOut state.")
            return true
        }
        return false
    }
                        ...
                        timestamp: Date? = nil)
    {
        ...
        if isInBackground {
            KhulnaSoftSessionManager.shared.resetSessionIfExpired {
                self.resetViews()
            }
        }
        ...
        let khulnasoftEvent = KhulnaSoftEvent(
            event: event,
        ...
        if snapshotEvent {
            ...
            replayQueue.add(khulnasoftEvent)
            return
        }
    ...
    private func isEnabled() -> Bool {
        if !enabled {
            hedgeLog("KhulnaSoft method was called without `setup` being complete. Call wil be ignored.")
        }
        return enabled
    }
    ...
    @objc public func close() {
        ...
        setupLock.withLock {
            enabled = false
            KhulnaSoftSDK.apiKeys.remove(config.apiKey)
            ...
            config = KhulnaSoftConfig(apiKey: "")
            featureFlags = nil
            storage = nil
            #if !os(watchOS)
                self.reachability?.stopNotifier()
    ...
    @objc public static func with(_ config: KhulnaSoftConfig) -> KhulnaSoftSDK {
        let khulnaSoft = KhulnaSoftSDK(config)
        khulnaSoft.setup(config)
        return khulnaSoft
    }
    ...
    @objc func handleAppDidBecomeActive() {
        KhulnaSoftSessionManager.shared.rotateSessionIdIfRequired {
            self.resetViews()
        }
    ...
    @objc func handleAppDidEnterBackground() {
        ...
        KhulnaSoftSessionManager.shared.updateSessionLastTime()
    ...
    func isSessionActive() -> Bool {
        ...
        return KhulnaSoftSessionManager.shared.isSessionActive()
    }
