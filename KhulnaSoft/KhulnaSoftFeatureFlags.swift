...
class KhulnaSoftFeatureFlags {
    private let config: KhulnaSoftConfig
    private let storage: KhulnaSoftStorage
    ...
    private let dispatchQueue = DispatchQueue(label: "com.khulnasoft.FeatureFlags",
                                              target: .global(qos: .utility))

    init(_ config: KhulnaSoftConfig,
         _ storage: KhulnaSoftStorage,
         _ api: KhulnaSoftApi)
    {
        self.config = config
    ...
    private func notifyAndRelease() {
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: KhulnaSoftSDK.didReceiveFeatureFlags, object: nil)
        }
    ...
    func getFeatureFlagPayload(_ key: String) -> Any? {
        ...
        do {
            ...
            // We need to mimic the JSON.parse of JS which is what khulnasoft-js uses
