...
class KhulnaSoftApi {
    ...
    func sessionConfig() -> URLSessionConfiguration {
        ...
        config.httpAdditionalHeaders = [
            ...
            "User-Agent": "\(khulnaSoftSdkName)/\(khulnaSoftVersion)",
        ]
    ...
    func batch(events: [KhulnaSoftEvent], completion: @escaping (KhulnaSoftBatchUploadInfo) -> Void) {
        guard let url = URL(string: "batch", relativeTo: config.host) else {
            hedgeLog("Malformed batch URL error.")
            return completion(KhulnaSoftBatchUploadInfo(statusCode: nil, error: nil))
        }
        ...
        do {
            ...
            return completion(KhulnaSoftBatchUploadInfo(statusCode: nil, error: error))
        }
        ...
        URLSession(configuration: config).uploadTask(with: request, from: gzippedPayload!) { data, response, error in
            ...
            return completion(KhulnaSoftBatchUploadInfo(statusCode: httpResponse.statusCode, error: error))
        }.resume()
    }

    func snapshot(events: [KhulnaSoftEvent], completion: @escaping (KhulnaSoftBatchUploadInfo) -> Void) {
        ...
        URLSession(configuration: config).uploadTask(with: request, from: data!) { data, response, error in
            ...
            if !(200 ... 299 ~= httpResponse.statusCode) {
                ...
                return completion(nil,
                                  InternalKhulnaSoftError(description: errorMessage))
            } else {
