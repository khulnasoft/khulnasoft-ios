...
class KhulnaSoftQueue {
    enum KhulnaSoftApiEndpoint: Int {
        case batch
        case snapshot
    }

    private let config: KhulnaSoftConfig
    private let api: KhulnaSoftApi
    ...
    private let endpoint: KhulnaSoftApiEndpoint
    ...
    private let fileQueue: KhulnaSoftFileBackedQueue

    #if !os(watchOS)
        init(_ config: KhulnaSoftConfig, _ storage: KhulnaSoftStorage, _ api: KhulnaSoftApi, _ endpoint: KhulnaSoftApiEndpoint, _ reachability: Reachability?) {
            self.config = config
            self.api = api
            ...
            switch endpoint {
            case .batch:
                fileQueue = KhulnaSoftFileBackedQueue(queue: storage.url(forKey: .queue), oldQueue: storage.url(forKey: .oldQeueue))
                dispatchQueue = DispatchQueue(label: "com.khulnasoft.Queue", target: .global(qos: .utility))
            case .snapshot:
                fileQueue = KhulnaSoftFileBackedQueue(queue: storage.url(forKey: .replayQeueue))
                dispatchQueue = DispatchQueue(label: "com.khulnasoft.ReplayQueue", target: .global(qos: .utility))
            }
        }
    #else
        init(_ config: KhulnaSoftConfig, _ storage: KhulnaSoftStorage, _ api: KhulnaSoftApi, _ endpoint: KhulnaSoftApiEndpoint) {
            self.config = config
            self.api = api
            self.endpoint = endpoint
    ...
    private func eventHandler(_ payload: KhulnaSoftConsumerPayload) {
        hedgeLog("Sending batch of \(payload.events.count) events to KhulnaSoft")
    ...
    private func handleResult(_ result: KhulnaSoftBatchUploadInfo, _ payload: KhulnaSoftConsumerPayload) {
        ...
        // TODO: https://github.com/KhulnaSoft/khulnasoft-android/pull/130
    ...
    func add(_ event: KhulnaSoftEvent) {
        ...
        do {
            ...
            hedgeLog("Tried to queue unserialisable KhulnaSoftEvent \(error)")
            return
        }
    ...
    private func take(_ count: Int, completion: @escaping (KhulnaSoftConsumerPayload) -> Void) {
        dispatchQueue.async {
            ...
            var processing = [KhulnaSoftEvent]()

            for item in items {
                // each element is a KhulnaSoftEvent if fromJSON succeeds
                guard let event = KhulnaSoftEvent.fromJSON(item) else {
                    continue
                }
                processing.append(event)
            }

            completion(KhulnaSoftConsumerPayload(events: processing) { success in
