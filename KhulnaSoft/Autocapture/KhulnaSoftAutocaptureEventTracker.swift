//
//  KhulnaSoftAutocaptureEventTracker.swift
//  KhulnaSoft
//
...
//
    ...
    class KhulnaSoftAutocaptureEventTracker {
        struct EventData {
        ...
        private static func registerNotifications() {
            NotificationCenter.default.addObserver(
                KhulnaSoftAutocaptureEventTracker.self,
        ...
        private static func unregisterNotifications() {
            NotificationCenter.default.removeObserver(KhulnaSoftAutocaptureEventTracker.self, name: UITextField.textDidEndEditingNotification, object: nil)
            NotificationCenter.default.removeObserver(KhulnaSoftAutocaptureEventTracker.self, name: UITextView.textDidEndEditingNotification, object: nil)
        }
    ...
    extension UIApplication {
        @objc func ph_swizzled_uiapplication_sendAction(_ action: Selector, to target: Any?, from sender: Any?, for event: UIEvent?) -> Bool {
            defer {
                ...
                {
                    KhulnaSoftAutocaptureEventTracker.eventProcessor?.process(source: .actionMethod(description: eventDescription), event: eventData)
                }
            }
    ...
    extension UIGestureRecognizer {
        ...
        @objc func ph_swizzled_uigesturerecognizer_state_Setter(_ state: UIGestureRecognizer.State) {
            ...
            if let eventData = view.eventData {
                KhulnaSoftAutocaptureEventTracker.eventProcessor?.process(source: .gestureRecognizer(description: gestureDescription), event: eventData)
            }
        }
    }
    ...
    extension UIScrollView {
        @objc func ph_swizzled_setContentOffset_Setter(_ newContentOffset: CGPoint) {
            ...
            if let eventData {
                KhulnaSoftAutocaptureEventTracker.eventProcessor?.process(source: .gestureRecognizer(description: EventType.kScroll), event: eventData)
            }
        }
    }
        ...
        @objc func ph_swizzled_setDelegate(_ delegate: (any UIPickerViewDelegate)?) {
            ...
            let forwardingDelegate = ForwardingPickerViewDelegate(delegate: delegate) { [weak self] in
                if let data = self?.eventData {
                    KhulnaSoftAutocaptureEventTracker.eventProcessor?.process(source: .gestureRecognizer(description: EventType.kValueChange), event: data)
                }
            }
    ...
    extension UIView {
        var eventData: KhulnaSoftAutocaptureEventTracker.EventData? {
            guard shouldTrack(self) else { return nil }
            return KhulnaSoftAutocaptureEventTracker.EventData(
    ...
    private extension UIView {
        var toElement: KhulnaSoftAutocaptureEventTracker.Element {
            KhulnaSoftAutocaptureEventTracker.Element(
                text: ph_autocaptureText.map(sanitizeText) ?? "",
                targetClass: descriptiveTypeName,
                baseClass: baseTypeName,
                label: khulnaSoftLabel
            )
        }
    }
    ...
    // see: https://github.com/KhulnaSoft/khulnasoft-js/blob/0cfffcac9bdf1da3fbb9478c1a51170a325bd57f/src/autocapture-utils.ts#L389
