//
//  KhulnaSoftStorageManagerTest.swift
//  KhulnaSoftTests
//
...
class KhulnaSoftStorageManagerTest: QuickSpec {
    func getSut() -> KhulnaSoftStorageManager {
        let config = KhulnaSoftConfig(apiKey: "123")
        return KhulnaSoftStorageManager(config)
    }
    ...
    override func spec() {
        ...
        it("Can can accept id customization via config") {
            ...
            let sut = KhulnaSoftStorageManager(config)
            let anonymousId = sut.getAnonymousId()
