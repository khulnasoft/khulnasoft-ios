...
class KhulnaSoftSDKTest: QuickSpec {
    func getSut(preloadFeatureFlags: Bool = false,
                sendFeatureFlagEvent: Bool = false,
                captureApplicationLifecycleEvents: Bool = false,
                flushAt: Int = 1,
                optOut: Bool = false,
                propertiesSanitizer: KhulnaSoftPropertiesSanitizer? = nil,
                personProfiles: KhulnaSoftPersonProfiles = .identifiedOnly) -> KhulnaSoftSDK
    {
        ...
        return KhulnaSoftSDK.with(config)
    }
    ...
    override func spec() {
        ...
        it("reset deletes khulnasoft files but not other folders") {
        ...
        it("captures $feature_flag_called when getFeatureFlag called twice after reloading flags") {
            ...
            context("autocapture") {
                it("isAutocaptureActive() should be false if disabled by config") {
                    let config = KhulnaSoftConfig(apiKey: "1234")
                    config.captureElementInteractions = false
                    let sut = KhulnaSoftSDK.with(config)
