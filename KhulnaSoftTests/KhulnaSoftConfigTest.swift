...
class KhulnaSoftConfigTest: QuickSpec {
    override func spec() {
        it("init config with default values") {
            let config = KhulnaSoftConfig(apiKey: "123")

            expect(config.host) == URL(string: KhulnaSoftConfig.defaultHost)
        ...
        it("init takes host") {
            let config = KhulnaSoftConfig(apiKey: "123", host: "localhost:9000")
            ...
            context("when initialized with default values for captureElementInteractions") {
                it("should enable autocapture by default") {
                    let sut = KhulnaSoftConfig(apiKey: "123")
