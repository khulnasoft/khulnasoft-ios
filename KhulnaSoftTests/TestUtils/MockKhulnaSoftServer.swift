...
class MockKhulnaSoftServer {
    ...
    func parseKhulnaSoftEvents(_ context: URLRequest) -> [KhulnaSoftEvent] {
        ...
        for event in batchEvents {
            guard let khulnasoftEvent = KhulnaSoftEvent.fromJSON(event) else { continue }
            events.append(khulnasoftEvent)
        }
