...
class KhulnaSoftStorageTest: QuickSpec {
    func getSut() -> KhulnaSoftStorage {
        let config = KhulnaSoftConfig(apiKey: "123")
        return KhulnaSoftStorage(config)
    }
    ...
    override func spec() {
        ...
        it("falls back to application support directory when app group identifier is not provided") {
            ...
            let sut = KhulnaSoftStorage(config)
