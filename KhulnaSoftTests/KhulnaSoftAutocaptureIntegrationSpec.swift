//
//  KhulnaSoftAutocaptureIntegrationSpec.swift
//  KhulnaSoft
//
...
import Quick
...
#if os(iOS)
    class KhulnaSoftAutocaptureIntegrationSpec: QuickSpec {
        override func spec() {
            var server: MockKhulnaSoftServer!
            var integration: KhulnaSoftAutocaptureIntegration!
            ...
            beforeEach {
                ...
                integration = KhulnaSoftAutocaptureIntegration(config)
                integration.start()
            }
            ...
            context("when initialized") {
                it("should set the eventProcessor to itself on start") {
                    integration.start()
                    expect(KhulnaSoftAutocaptureEventTracker.eventProcessor).to(beIdenticalTo(integration))
                }

                it("should clear the eventProcessor on stop") {
                    integration.start()
                    integration.stop()
                    expect(KhulnaSoftAutocaptureEventTracker.eventProcessor).to(beNil())
                }
            }
    ...
    private func createTestEventData(debounceInterval: TimeInterval = 0) -> KhulnaSoftAutocaptureEventTracker.EventData {
        KhulnaSoftAutocaptureEventTracker.EventData(
